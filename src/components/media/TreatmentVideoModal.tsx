...
import { Play, Pause, Volume2, VolumeX, Maximize, X, CheckCircle, Stethoscope, ShieldCheck, Activity, Film } from 'lucide-react';
...
export interface TreatmentVideo {
  id: string;
  title: string;
  category: 'PHC Care' | 'District Teleconsult' | 'Cath Lab Specialist' | 'Emergency 108';
  duration: string;
  description: string;
  videoUrl: string;
  posterUrl: string;
...
export const treatmentVideosList: TreatmentVideo[] = [
  {
    id: 'VID-01',
...
interface TreatmentVideoModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialVideoId?: string;
}

export const TreatmentVideoModal: React.FC<TreatmentVideoModalProps> = ({
  isOpen,
  onClose,
}) => {
  const selectedId = initialVideoId || 'VID-01';
  const [activeVideo, setActiveVideo] = useState<TreatmentVideo>(
    treatmentVideosList.find(v => v.id === selectedId) || treatmentVideosList[0]
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="3D & Video Medical Treatment Demonstration" size="lg">
      <div className="space-y-6">
        ...
        <div className="relative rounded-2xl overflow-hidden bg-slate-950 border border-slate-800 shadow-2xl aspect-video group">
          {!videoError ? (
            ...
            <div className="relative w-full h-full bg-slate-900 flex flex-col items-center justify-center p-6 text-center space-y-4">
              <img
          ...
          {!isPlaying && (
            <button
              onClick={togglePlay}
              className="absolute inset-0 flex items-center justify-center bg-slate-950/40 hover:bg-slate-950/20 transition-all z-10"
            >
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between z-10">
            <div className="flex items-center gap-3">
              <button
                onClick={togglePlay}
                className="p-2 rounded-xl bg-slate-900/90 text-white hover:bg-emerald-600 transition-colors border border-slate-700"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
              {activeVideo.steps.map(s => (
                <div key={s.step} className="p-3 rounded-lg bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-800 space-y-1">
