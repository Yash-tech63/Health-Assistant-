...
export const NearestHospitals = ({ onSelectHospital, className = '', }) => {
    return (<div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {mockHospitals.map((hosp) => (<Card key={hosp.id} hoverable className="flex flex-col justify-between border-t-4 border-t-medical-500">
            <CardBody className="space-y-4 flex-1 flex flex-col justify-between">
              ...
              <div className="pt-3 flex items-center gap-2 border-t border-slate-100 dark:border-slate-800">
                <a href={`tel:${hosp.phone}`} className="flex-1 px-3 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors border border-emerald-200 dark:border-emerald-800">
                  <PhoneCall className="h-3.5 w-3.5"/>
                  <span>Call Desk</span>
                </a>

                <Button variant="outline" size="sm" onClick={() => onSelectHospital && onSelectHospital(hosp)} className="flex-1 text-xs">
