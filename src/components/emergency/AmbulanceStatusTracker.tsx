...
export type AmbulanceStatusStep = 'received' | 'assigned' | 'on_the_way' | 'arrived';

interface AmbulanceStatusTrackerProps {
  currentStep?: AmbulanceStatusStep;
  requestId?: string;
  className?: string;
}

export const AmbulanceStatusTracker: React.FC<AmbulanceStatusTrackerProps> = ({
  currentStep = 'on_the_way',
  requestId = '#AMB-1024',
  className = '',
}) => {
  const steps: { id: AmbulanceStatusStep; label: string; sub: string; icon: React.ReactNode }[] = [
    {
      id: 'received',
  ...
  const stepOrder: AmbulanceStatusStep[] = ['received', 'assigned', 'on_the_way', 'arrived'];
  const currentIndex = stepOrder.indexOf(currentStep);

  return (
    <div className={`p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className="relative flex md:flex-col items-center gap-3.5 md:gap-2 z-10">
              {/* Step Circle */}
              <div
                className={`
