...
import { MapPin, Loader2, Check, AlertTriangle, Phone, User } from 'lucide-react';
export const EmergencyRequestModal = ({ isOpen, onClose, onSuccess, }) => {
    ...
    return (<Modal isOpen={isOpen} onClose={handleResetModal} title="🚑 Request Emergency Ambulance" size="lg">
      <div className="space-y-6">
        <p className="text-xs text-slate-500">
          Provide the emergency details below so assistance can be arranged immediately.
        </p>

        {step === 'loading' && (<div className="py-12 text-center space-y-4">
        ...
        {step === 'confirm' && (<div className="space-y-6">
            ...
            <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 text-xs">
              ...
              {formData.description && (<div className="flex justify-between pt-1">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <Button variant="danger" onClick={handleConfirmSubmission} className="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 font-bold">
                🚑 CONFIRM AMBULANCE REQUEST
              </Button>
            </div>
          </div>)}

        {step === 'form' && (<form onSubmit={handleNextToConfirm} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Patient Name */}
              <Input label="Patient Name *" placeholder="Enter patient name" value={formData.patientName} onChange={(e) => handleInputChange('patientName', e.target.value)} error={errors.patientName} leftIcon={<User className="h-4 w-4 text-slate-400"/>}/>

              {/* Mobile Number */}
              <Input label="Mobile Number *" placeholder="Enter mobile number" value={formData.mobileNumber} onChange={(e) => handleInputChange('mobileNumber', e.target.value)} error={errors.mobileNumber} leftIcon={<Phone className="h-4 w-4 text-slate-400"/>}/>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <select value={formData.emergencyType} onChange={(e) => handleInputChange('emergencyType', e.target.value)} className={`
                ...
                {emergencyTypes.map((type) => (<option key={type} value={type}>
                    {type}
                  </option>))}
              </select>
              {errors.emergencyType && (<p className="text-[10px] font-bold text-red-500 mt-1">{errors.emergencyType}</p>)}
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={handleUseCurrentLocation} disabled={locating} className="text-xs font-bold text-rose-600 dark:text-rose-400 hover:underline flex items-center gap-1 transition-colors disabled:opacity-50">
                  {locating ? (<Loader2 className="h-3.5 w-3.5 animate-spin"/>) : (<MapPin className="h-3.5 w-3.5"/>)}
                  <span>📍 Use My Current Location</span>
                </button>
              </div>

              <Input placeholder="Enter pickup location address or landmark..." value={formData.pickupLocation} onChange={(e) => handleInputChange('pickupLocation', e.target.value)} error={errors.pickupLocation} leftIcon={<MapPin className="h-4 w-4 text-slate-400"/>}/>

              {locationMessage && (<p className={`text-[10px] font-semibold mt-1 flex items-center gap-1 ${locationMessage.includes('Unable')
                    ? 'text-amber-600 dark:text-amber-400'
                    : 'text-emerald-600 dark:text-emerald-400'}`}>
                  {locationMessage.includes('Unable') ? (<AlertTriangle className="h-3 w-3"/>) : (<Check className="h-3 w-3"/>)}
            ...
            <div className="space-y-1.5">
              ...
              <textarea rows={3} placeholder="Briefly describe the emergency condition or symptoms..." value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl text-xs font-medium bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-500/40 focus:border-rose-500 transition-colors resize-none"/>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              ...
              <Button type="submit" variant="danger" className="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 font-bold">
                Proceed to Review
              </Button>
            </div>
          </form>)}
