...
import { Truck, Building2, MapPin, PhoneCall, Check, Loader2 } from 'lucide-react';
import { Card, CardBody } from '../Card';

interface EmergencyQuickActionsProps {
  onRequestAmbulance: () => void;
  onFindHospital: () => void;
  onShareLocation?: (locationStr: string) => void;
  className?: string;
}
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (onShareLocation) onShareLocation(coords);
        },
        (error) => {
          setLocating(false);
          ...
          if (onShareLocation) onShareLocation(fallback);
        },
        { timeout: 8000 }
      );
    } else {
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card 
          ...
          className="border-l-4 border-l-rose-500 hover:shadow-lg transition-all group"
        >
        ...
        <Card 
          ...
          className="border-l-4 border-l-teal-500 hover:shadow-lg transition-all group"
        >
          <CardBody className="p-4 flex items-start gap-3.5">
        ...
        <Card 
          ...
          className="border-l-4 border-l-amber-500 hover:shadow-lg transition-all group"
        >
          <CardBody className="p-4 flex items-start gap-3.5">
