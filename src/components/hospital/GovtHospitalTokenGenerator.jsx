...
import { Ticket, Clock, MapPin, Printer, Landmark } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
export const govtHospitalsList = [
    {
        id: 'GH-01',
...
export const GovtHospitalTokenGenerator = () => {
    const { user } = useAuth();
    const [selectedHospital, setSelectedHospital] = useState(govtHospitalsList[0]);
    ...
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <Card className="border-t-4 border-t-rose-600 shadow-md">
            ...
            <CardBody className="space-y-4">
              <form onSubmit={handleGenerateToken} className="space-y-4">
                ...
                <div>
                  ...
                  <select value={selectedHospital.id} onChange={e => {
            ...
            if (found)
                setSelectedHospital(found);
                    ...
                    {govtHospitalsList.map(h => (<option key={h.id} value={h.id}>
                        {h.name} ({h.type})
                      </option>))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select value={department} onChange={e => setDepartment(e.target.value)} className="w-full text-xs rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-3 outline-none focus:ring-2 focus:ring-rose-500">
        ...
        <div className="lg:col-span-7 space-y-6">
          {generatedToken ? (<Card className="border-2 border-rose-500 shadow-xl overflow-hidden bg-white dark:bg-slate-900">
            ...
            </Card>) : (<div className="bg-slate-100 dark:bg-slate-900/60 p-8 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 text-center space-y-3">
      ...
      <div className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {govtHospitalsList.map(h => (<Card key={h.id} className="overflow-hidden hover:shadow-lg transition-all group border-slate-200 dark:border-slate-800">
              <div className="relative h-44 w-full overflow-hidden bg-slate-900">
                <img src={h.image} alt={h.name} onError={(e) => {
              ...
              <CardBody className="p-4 space-y-2">
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-[10px]">
                  ...
                  <button onClick={() => setSelectedHospital(h)} className="text-rose-600 dark:text-rose-400 font-bold hover:underline">
                    Select for Token
                  </button>
                </div>
              </CardBody>
