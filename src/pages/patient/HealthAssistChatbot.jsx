...
import { Bot, Send, ArrowRight, Activity } from 'lucide-react';
export const HealthAssistChatbot = () => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <Card className="lg:col-span-3 h-[520px] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            return (<div key={m.id} className={`flex ${isBot ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-xs sm:max-w-xl rounded-2xl px-4 py-2.5 text-xs border shadow-xs ${isBot
          ...
          {/* Chat input footer */}
          <form onSubmit={(e) => { e.preventDefault(); handleSend(inputText); }} className="p-4 border-t border-slate-150 dark:border-slate-850 bg-slate-50/20 dark:bg-slate-900/10 flex gap-2">
            <Input id="chatbot-input" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Ask about chest pain, high fever, or Ayushman card benefits..." className="flex-1" autoComplete="off" required/>
        ...
        <div className="space-y-4 col-span-1">
          ...
          <div className="space-y-3">
            {promptChips.map((chip, idx) => (<button key={idx} onClick={() => handleSend(chip.query)} className="w-full text-left p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 text-xs font-semibold hover:border-emerald-500 hover:bg-emerald-50/10 transition-all flex items-start justify-between gap-2">
