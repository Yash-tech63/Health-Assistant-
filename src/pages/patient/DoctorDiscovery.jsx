...
export const DoctorDiscovery = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700/50 shadow-sm grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Input id="disc-search" placeholder="Search by physician name..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} leftIcon={<Search className="h-4 w-4"/>}/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredDoctors.map(doc => (<Card key={doc.id}>
      ...
      {/* Booking Form Modal */}
      {bookingDoc && (<Modal isOpen={!!bookingDoc} onClose={() => setBookingDoc(null)} title={`Book Consultation: ${bookingDoc.name}`}>
          <form onSubmit={handleBook} className="space-y-4">
            ...
            <Input id="book-date" label="Select Appointment Date" type="date" value={bookingDate} onChange={e => setBookingDate(e.target.value)} required/>

            <div>
              <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Time Slot</label>
              <select value={bookingSlot} onChange={e => setBookingSlot(e.target.value)} className="w-full text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2.5 outline-none focus:ring-2 focus:ring-medical-500">
            ...
            <div>
              ...
              <select value={bookingType} onChange={e => setBookingType(e.target.value)} className="w-full text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2.5 outline-none focus:ring-2 focus:ring-medical-500">
      ...
      <Modal isOpen={successModalOpen} onClose={() => setSuccessModalOpen(false)} title="Appointment Confirmed">
