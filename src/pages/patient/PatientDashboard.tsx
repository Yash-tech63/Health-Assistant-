...
import { Calendar, FileText, Pill, ArrowRight, ShieldCheck, MapPin, Activity, Stethoscope, Play, Landmark } from 'lucide-react';
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-slate-800 shadow-xs">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button
            ...
            className="bg-rose-600 hover:bg-rose-700 text-white font-bold self-center shadow-md py-3"
          >
          ...
          <Button
            ...
            leftIcon={<Landmark className="h-4 w-4 text-rose-400" />}
            className="border-slate-700 text-slate-200 hover:bg-slate-800 font-bold self-center shadow-md py-3"
          >
          ...
          <Card className="border-l-4 border-l-teal-600">
            ...
            <CardBody className="p-6">
              <div className="relative pl-6 border-l-2 border-slate-100 dark:border-slate-800 space-y-6">
                {journeyTiers.map((j, idx) => (
                  <div key={idx} className="relative">
                    {/* Circle indicators */}
                    <div className={`absolute -left-[31px] top-1.5 h-4.5 w-4.5 rounded-full border-2 flex items-center justify-center text-[9px] font-bold ${
                      j.status === 'Completed' 
            ...
            <CardBody className="p-0">
              <div className="divide-y divide-slate-100 dark:divide-slate-850">
                {myAppointments.map(a => (
                  <div key={a.id} className="p-4 space-y-2 text-xs hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
