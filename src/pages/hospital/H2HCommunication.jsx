      ...
      <Card className="h-[500px] flex flex-col overflow-hidden">
        ...
        <div className="flex-grow overflow-y-auto p-6 space-y-4">
          {messages.map(msg => {
            ...
            return (<div key={msg.id} className={`flex ${isLocal ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs sm:max-w-xl rounded-2xl px-4 py-3 text-xs border shadow-xs ${isLocal
        ...
        <form onSubmit={handleSend} className="p-4 border-t border-slate-150 dark:border-slate-850 bg-slate-50/20 dark:bg-slate-900/10 flex gap-2">
          <Input id="h2h-input" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Type bed reservation request or dispatch notice..." className="flex-1" autoComplete="off" required/>
