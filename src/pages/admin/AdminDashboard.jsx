...
import { ShieldCheck, Users, Stethoscope, Building, ClipboardCheck, Siren, Search, Phone } from 'lucide-react';
import { initialAmbulances } from '../../data/mockData';
export const AdminDashboard = () => {
    ...
    // Filtered queries
    const filteredPatients = patients.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase()) || p.abhaId.includes(searchTerm));
    ...
    const filteredFacilities = facilities.filter(f => f.name.toLowerCase().includes(searchTerm.toLowerCase()) || f.location.toLowerCase().includes(searchTerm.toLowerCase()));
    const filteredAmbulances = ambulances.filter(a => a.vehicleNo.toLowerCase().includes(searchTerm.toLowerCase()) || a.driverName.toLowerCase().includes(searchTerm.toLowerCase()) || a.stationFacilityName.toLowerCase().includes(searchTerm.toLowerCase()));
    const filteredLogs = auditLogs.filter(l => l.action.toLowerCase().includes(searchTerm.toLowerCase()) || l.details.toLowerCase().includes(searchTerm.toLowerCase()) || l.actorName.toLowerCase().includes(searchTerm.toLowerCase()));
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <Card className={`cursor-pointer transition-all ${activeTab === 'patients' ? 'ring-2 ring-emerald-500 border-emerald-500 bg-emerald-50/20' : 'hover:shadow-md'}`} onClick={() => setActiveTab('patients')}>
        ...
        <Card className={`cursor-pointer transition-all ${activeTab === 'doctors' ? 'ring-2 ring-emerald-500 border-emerald-500 bg-emerald-50/20' : 'hover:shadow-md'}`} onClick={() => setActiveTab('doctors')}>
          <CardBody className="p-4 flex items-center justify-between">
            <div>
        ...
        <Card className={`cursor-pointer transition-all ${activeTab === 'hospitals' ? 'ring-2 ring-emerald-500 border-emerald-500 bg-emerald-50/20' : 'hover:shadow-md'}`} onClick={() => setActiveTab('hospitals')}>
          <CardBody className="p-4 flex items-center justify-between">
            <div>
        ...
        <Card className={`cursor-pointer transition-all ${activeTab === 'ambulances' ? 'ring-2 ring-emerald-500 border-emerald-500 bg-emerald-50/20' : 'hover:shadow-md'}`} onClick={() => setActiveTab('ambulances')}>
          <CardBody className="p-4 flex items-center justify-between">
            <div>
        ...
        <Card className={`cursor-pointer transition-all ${activeTab === 'activities' ? 'ring-2 ring-emerald-500 border-emerald-500 bg-emerald-50/20' : 'hover:shadow-md'}`} onClick={() => setActiveTab('activities')}>
          <CardBody className="p-4 flex items-center justify-between">
            <div>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-xs flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <button onClick={() => setActiveTab('patients')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'patients' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200'}`}>
            <Users className="h-4 w-4"/> Patient Details
          </button>
          <button onClick={() => setActiveTab('doctors')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'doctors' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200'}`}>
            <Stethoscope className="h-4 w-4"/> Doctor Details
          </button>
          <button onClick={() => setActiveTab('hospitals')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'hospitals' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200'}`}>
            <Building className="h-4 w-4"/> Hospital Details
          </button>
          <button onClick={() => setActiveTab('ambulances')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'ambulances' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200'}`}>
            <Siren className="h-4 w-4 text-rose-300"/> Ambulance Details
          </button>
          <button onClick={() => setActiveTab('activities')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeTab === 'activities' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200'}`}>
        ...
        <div className="w-full md:w-64">
          <Input id="admin-search" placeholder={`Search ${activeTab}...`} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} leftIcon={<Search className="h-4 w-4"/>}/>
        </div>
      </div>

      {/* Tab 1: PATIENT DETAILS */}
      {activeTab === 'patients' && (<Card>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {filteredPatients.map(p => (<tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {p.avatar ? (<img src={p.avatar} alt={p.name} className="h-10 w-10 rounded-full object-cover border border-slate-200 shrink-0"/>) : (<span className="text-2xl">👨‍🌾</span>)}
                          <div>
          ...
          <CardBody className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {filteredDoctors.map(d => (<tr key={d.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {d.avatar ? (<img src={d.avatar} alt={d.name} className="h-10 w-10 rounded-full object-cover border border-slate-200 shrink-0"/>) : (<span className="text-2xl">👨‍⚕️</span>)}
                          <div>
          ...
          <CardBody className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {filteredFacilities.map(f => (<tr key={f.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {f.image ? (<img src={f.image} alt={f.name} className="h-10 w-10 rounded-xl object-cover border border-slate-200 shrink-0"/>) : (<span className="text-2xl">🏥</span>)}
          ...
          <CardBody className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {filteredAmbulances.map(a => (<tr key={a.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                      ...
                      <td className="px-6 py-4">
                        {a.status === 'Dispatched' ? (<div className="space-y-1">
                          ...
                          </div>) : a.status === 'En-Route Hospital' ? (<div className="space-y-1">
                          ...
                          </div>) : (<Badge color="success">🟢 Station Ready</Badge>)}
                      </td>
                    </tr>))}
                </tbody>
              </table>
          ...
          <CardBody className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {filteredLogs.map(log => (<tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
