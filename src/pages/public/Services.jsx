...
export const Services = () => {
    ...
    return (<div className="max-w-5xl mx-auto px-4 py-12 space-y-12">
      ...
      <div className="space-y-8">
        {serviceCategories.map((cat, idx) => (<Card key={idx} className="border-l-4 border-l-medical-600">
            ...
            <CardBody>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {cat.items.map((item, idy) => (<li key={idy} className="flex items-start gap-2.5 text-xs text-slate-650 dark:text-slate-350">
      ...
      <div className="space-y-6 pt-6 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {treatmentVideosList.map(v => (<Card key={v.id} className="hover:shadow-lg transition-all group overflow-hidden border-slate-200 dark:border-slate-800">
      ...
      {videoModalOpen && (<TreatmentVideoModal isOpen={videoModalOpen} onClose={() => setVideoModalOpen(false)} initialVideoId={selectedVidId}/>)}
