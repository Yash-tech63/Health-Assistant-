...
export const HospitalSearch = () => {
    ...
    return (<div className="max-w-6xl mx-auto px-4 py-12 space-y-12">
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/50 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-4">
        <Input id="hosp-search" placeholder="Search by facility name or location..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} leftIcon={<Search className="h-4 w-4"/>}/>

        <div>
          <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Filter Tier Classification</label>
          <select value={selectedType} onChange={e => setSelectedType(e.target.value)} className="w-full text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2.5 outline-none focus:ring-2 focus:ring-medical-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredHospitals.map(fac => {
            const occupancyRate = fac.totalBeds && fac.bedsAvailable
                ...
                : 0;
            return (<Card key={fac.id} className="hover:shadow-md transition-all">
              <CardBody className="space-y-4">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex gap-3">
                    {fac.image ? (<img src={fac.image} alt={fac.name} className="h-14 w-14 rounded-2xl object-cover border border-slate-200 dark:border-slate-800 shadow-xs shrink-0"/>) : (<div className="text-3xl bg-slate-50 dark:bg-slate-900 p-3 h-14 w-14 rounded-2xl flex items-center justify-center border border-slate-100 dark:border-slate-800 shrink-0">
                        🏥
                      </div>)}
                    <div>
                ...
                {fac.totalBeds && (<div className="space-y-2 pt-2 border-t border-slate-50 dark:border-slate-900">
                    ...
                    <div className="w-full bg-slate-100 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
                      <div className={`h-full transition-all duration-300 ${occupancyRate > 85 ? 'bg-red-500' : occupancyRate > 60 ? 'bg-amber-500' : 'bg-hospital-500'}`} style={{ width: `${100 - occupancyRate}%` }}/>
                    </div>
                  </div>)}
              </CardBody>
            </Card>);
        ...
        {filteredHospitals.length === 0 && (<div className="col-span-full text-center py-12 bg-white dark:bg-slate-800 rounded-2xl border border-dashed border-slate-250 dark:border-slate-700">
