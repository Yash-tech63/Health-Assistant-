...
export const Register = () => {
    ...
    const [step, setStep] = useState(1); // Step 1: Info, Step 2: ABHA Generation
    ...
    const handleNextStep = (e) => {
        ...
        // Generate simulated 14 digit ABHA ID (format: XX-XXXX-XXXX-XXXX)
    ...
    const handleFinalize = () => {
        setLoading(true);
        setTimeout(() => {
            // Create Patient in health store
            createPatient({
                name,
                age: parseInt(age),
                gender,
            ...
            // Login immediately
            login('patient@swasthya.in', 'patient');
            setLoading(false);
    ...
    return (<div className="max-w-6xl mx-auto px-4 py-12 space-y-10">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-500">
          Create your Indian digital health record profile to secure seamless referrals and track medical prescriptions across health tiers.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        
        {/* Left Column: ABHA Registration Form Card (5 cols) */}
          ...
          <Card className="shadow-lg border-t-4 border-t-emerald-600">
            <CardBody className="space-y-6">
              ...
              {step === 1 ? (<form onSubmit={handleNextStep} className="space-y-4">
                  <Input id="reg-name" label="Full Name (Aadhaar Match)" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Ramesh Kumar" required/>

                  <div className="grid grid-cols-2 gap-4">
                    <Input id="reg-age" label="Age" type="number" value={age} onChange={e => setAge(e.target.value)} placeholder="e.g. 35" required/>
                    
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Gender</label>
                      <select value={gender} onChange={e => setGender(e.target.value)} className="w-full text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2.5 outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Input id="reg-phone" label="Aadhaar-linked Phone" placeholder="+91 XXXXX XXXXX" value={phone} onChange={e => setPhone(e.target.value)} required/>

                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Blood Group</label>
                      <select value={bloodGroup} onChange={e => setBloodGroup(e.target.value)} className="w-full text-sm rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2.5 outline-none focus:ring-2 focus:ring-emerald-500">
                  ...
                  <Input id="reg-loc" label="District Location (City/Village)" placeholder="e.g. Sunni, Shimla, Himachal Pradesh" value={location} onChange={e => setLocation(e.target.value)} required/>

                  <Button type="submit" variant="primary" className="w-full font-bold py-3">
                    Verify Aadhaar & Generate ABHA
                  </Button>
                </form>) : (<div className="space-y-6 text-center">
        ...
        {/* Right Column: Embedded About Us & Healthcare Tier Overview (7 cols) */}
        <div className="lg:col-span-7 space-y-6">
          
          {/* About Us Ecosystem Overview Banner */}
          ...
          {/* The 3-Tier Healthcare Grid Cards */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="border-t-4 border-t-teal-500">
              <CardBody className="p-4 space-y-2">
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 bg-teal-50 dark:bg-teal-950/40 text-teal-600 rounded-lg flex items-center justify-center font-bold text-xs">
                    T1
                  </div>
            ...
            <Card className="border-t-4 border-t-indigo-500">
              <CardBody className="p-4 space-y-2">
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 rounded-lg flex items-center justify-center font-bold text-xs">
                    T2
                  </div>
            ...
            <Card className="border-t-4 border-t-rose-500">
              <CardBody className="p-4 space-y-2">
                <div className="flex items-center gap-2">
                  <div className="h-8 w-8 bg-rose-50 dark:bg-rose-950/40 text-rose-600 rounded-lg flex items-center justify-center font-bold text-xs">
                    T3
                  </div>
