...
import { Sun, Moon, Globe, Shield, HeartPulse, Menu, X } from 'lucide-react';
...
export const PublicLayout = () => {
    ...
    return (<div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 transition-colors duration-200 relative overflow-hidden">
      <HealthcareBackground />
      {/* Quick Access Portal Switcher Bar */}
      {showRoleSwitcher && (<div className="bg-gradient-to-r from-rose-600 via-pink-600 to-rose-700 text-white py-2 px-4 text-xs">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {rolesList.map(r => (<button key={r.role} onClick={() => handleQuickSwitch(r.role)} className={`px-2 py-1 rounded bg-white/10 hover:bg-white/20 transition-all font-semibold flex items-center gap-1 ${user?.role === r.role ? 'ring-2 ring-white bg-white/20' : ''}`}>
                  <span>{r.icon}</span>
                  <span>{r.label}</span>
                </button>))}
              <button onClick={() => setShowRoleSwitcher(false)} className="ml-2 hover:opacity-80" title="Hide simulation bar">
      ...
      <header className="sticky top-0 z-40 bg-white/85 dark:bg-slate-900/85 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/80 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            return (<Link key={item.name} to={item.href} className={`text-sm font-medium transition-colors ${isActive
            ...
            <div className="hidden md:flex items-center space-x-4">
              {/* Language Switcher */}
              <button onClick={() => setLanguage(language === 'en' ? 'hi' : 'en')} className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 dark:hover:text-medical-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex items-center gap-1.5" title="Toggle Language">
                <Globe className="h-4 w-4"/>
                <span className="text-xs font-semibold uppercase">{language}</span>
              </button>

              {/* Theme Toggle */}
              <button onClick={toggleTheme} className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 dark:hover:text-medical-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" title="Toggle Dark Mode">
                {theme === 'dark' ? <Sun className="h-4 w-4"/> : <Moon className="h-4 w-4"/>}
              </button>

              {/* Auth Buttons */}
              {user ? (<div className="flex items-center gap-3">
                  <Button variant="outline" size="sm" onClick={() => navigate(`/portal/${user.role}`)} leftIcon={<span>{user.avatar}</span>}>
                    {t('nav.dashboard')}
                  </Button>
            ...
            <div className="md:hidden flex items-center gap-3">
              ...
              <button onClick={() => setLanguage(language === 'en' ? 'hi' : 'en')} className="p-1 text-slate-500 dark:text-slate-400">
                <Globe className="h-4 w-4 inline mr-1"/>
                <span className="text-xs font-bold uppercase">{language}</span>
              </button>
              
              {/* Mobile Theme Toggle */}
              <button onClick={toggleTheme} className="p-1 text-slate-500 dark:text-slate-400">
                {theme === 'dark' ? <Sun className="h-4 w-4"/> : <Moon className="h-4 w-4"/>}
              </button>

              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
        ...
        {/* Mobile Menu Drawer */}
        {mobileMenuOpen && (<div className="md:hidden border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-3 space-y-3 shadow-lg transition-all">
            <nav className="flex flex-col space-y-2">
              {navigation.map((item) => (<Link key={item.name} to={item.href} onClick={() => setMobileMenuOpen(false)} className="px-3 py-2 rounded-lg text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 hover:text-medical-600 dark:hover:text-medical-400">
                  {item.name}
                </Link>))}
            </nav>
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
              ...
              <div className="grid grid-cols-2 gap-2 px-2">
                {rolesList.map(r => (<button key={r.role} onClick={() => handleQuickSwitch(r.role)} className="text-xs py-2 px-2.5 rounded-lg border border-slate-200 dark:border-slate-800 text-left bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-medium flex items-center gap-1.5 hover:bg-medical-50 dark:hover:bg-medical-950/20">
                    <span>{r.icon}</span>
