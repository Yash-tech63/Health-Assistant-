...
import {
  Home, User, Calendar, FileText, ClipboardList, Pill, MessageSquare,
  Bot, Settings, LogOut, Sun, Moon, Globe, Menu, X, ArrowLeft,
  Users, ShieldCheck, HeartPulse, Building2, Bell, ShieldAlert, Video
} from 'lucide-react';
...
export const DashboardLayout: React.FC = () => {
  ...
  const roleLinks: Record<UserRole, { label: string; href: string; icon: React.ReactNode }[]> = {
    patient: [
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white dark:bg-slate-950 border-r border-slate-100 dark:border-slate-850/80 transition-colors z-10">
          ...
          <div className="flex items-center gap-3">
            {user.avatar && user.avatar.startsWith('http') ? (
              <img 
                ...
                className="h-10 w-10 rounded-xl object-cover border border-slate-200 dark:border-slate-800 shadow-xs shrink-0"
              />
            ) : (
              <div className="text-2xl bg-white dark:bg-slate-800 p-1.5 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xs shrink-0">
                👨‍⚕️
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {(['patient', 'doctor', 'hospital', 'admin'] as UserRole[]).map(r => (
              <button
                ...
                className={`text-[10px] py-1 px-1.5 rounded border text-center transition-all ${
                  role === r
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-850/80">
          <button
            ...
            className="flex items-center gap-3 w-full px-3 py-2 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-xl transition-all"
          >
        ...
        <header className="h-16 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-850/80 flex items-center justify-between px-4 sm:px-6 transition-colors">
          <div className="flex items-center gap-4">
            <button
              ...
              className="md:hidden p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400"
            >
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="relative">
              <button
                ...
                className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 dark:hover:text-medical-400 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors relative"
              >
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl shadow-xl z-50">
                  <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                    ...
                    <button 
                      ...
                      className="text-[10px] text-medical-600 dark:text-medical-400 hover:underline font-semibold"
                    >
                      Mark all read
                    </button>
                  </div>
                  <div className="max-h-60 overflow-y-auto">
                    {notifications.map(n => (
                      <div 
                        key={n.id} 
                        className={`p-3 border-b border-slate-50 dark:border-slate-900 text-xs transition-colors hover:bg-slate-50 dark:hover:bg-slate-900 ${
                          n.unread ? 'bg-medical-50/20 dark:bg-medical-950/10' : ''
                        }`}
                      >
            ...
            <button
              ...
              className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors text-xs font-bold uppercase"
            >
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-lg text-slate-500 hover:text-medical-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors"
            >
            ...
            <div className="flex items-center gap-2 border-l border-slate-100 dark:border-slate-800 pl-3 sm:pl-4">
              {user.avatar && user.avatar.startsWith('http') ? (
                <img
                  ...
                  className="h-7 w-7 rounded-full object-cover border border-slate-200 dark:border-slate-800 shadow-xs shrink-0"
                />
              ) : (
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-850/80 bg-slate-50/50 dark:bg-slate-900/30">
          <div className="flex items-center gap-3">
            {user.avatar && user.avatar.startsWith('http') ? (
              ...
              <div className="text-3xl bg-white dark:bg-slate-800 p-1.5 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xs">
                👨‍⚕️
              </div>
            )}
            <div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {(['patient', 'doctor', 'hospital', 'admin'] as UserRole[]).map(r => (
              <button
                ...
                className={`text-[10px] py-1.5 px-1.5 rounded border text-center transition-all ${
                  role === r
                    ? 'bg-medical-600 border-medical-600 text-white font-bold'
