...
import { Phone, MapPin, User, Compass } from 'lucide-react';
import { Button } from '../Button';
export const AmbulanceTrackingCard = ({ ambulanceId = 'AMB-204', eta = '10 min', distance = '2.8 km', driverName = 'Vikram Singh (Certified Paramedic)', driverPhone = '+91 98160 11223', pickupLocation = 'Dhami Village, Sector 4', onTrackClick, className = '', }) => {
    return (<div className={`p-6 bg-gradient-to-br from-slate-900 to-slate-950 text-white rounded-2xl border border-slate-800 shadow-xl space-y-6 relative overflow-hidden ${className}`}>
      ...
      <div className="space-y-3 text-xs">
        <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/50 border border-slate-800">
          ...
          <a href={`tel:${driverPhone}`} className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-1.5 transition-colors shadow-xs">
      ...
      <div className="pt-2 flex flex-col sm:flex-row gap-3">
        <Button variant="primary" onClick={onTrackClick} className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-bold border-none shadow-md shadow-amber-500/10 flex items-center justify-center gap-2">
