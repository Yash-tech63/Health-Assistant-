...
import { Mic, MicOff, Video as VideoIcon, VideoOff, Monitor, MessageSquare, PhoneOff, Heart, Activity, Thermometer, ShieldCheck, X, Send, FileText, CheckCircle } from 'lucide-react';
import { Button } from '../Button';
import { Badge } from '../Badge';
export const VideoConsultationRoom = ({ doctorName, doctorSpecialty, doctorFacility, patientName, abhaId, userRole, onEndCall, }) => {
    ...
    return (<div className="relative w-full h-[calc(100vh-6rem)] bg-slate-950 rounded-3xl overflow-hidden shadow-2xl flex flex-col border border-slate-800">
      ...
      <div className="relative flex-1 bg-slate-900 overflow-hidden flex items-center justify-center">
        ...
        {/* Main Peer Feed */}
        {cameraActive ? (<img src={mainVideo} alt="Live Teleconsultation Feed" className="w-full h-full object-cover"/>) : (<div className="flex flex-col items-center justify-center text-slate-500 space-y-2">
        ...
        <div className="absolute bottom-4 right-4 w-36 sm:w-48 h-24 sm:h-32 rounded-2xl overflow-hidden border-2 border-slate-700 shadow-2xl bg-slate-950 z-10">
          <img src={selfVideo} alt="Self Camera Feed" className="w-full h-full object-cover"/>
        ...
        {chatOpen && (<div className="absolute top-0 right-0 bottom-0 w-80 bg-slate-900/95 backdrop-blur-lg border-l border-slate-800 p-4 flex flex-col z-20 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto py-3 space-y-3">
              {messages.map((msg, idx) => (<div key={idx} className="bg-slate-800/80 rounded-xl p-2.5 border border-slate-700/50 space-y-1">
            ...
            <form onSubmit={handleSendMessage} className="pt-2 border-t border-slate-800 flex gap-2">
              <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Type a clinical note or message..." className="flex-1 bg-slate-800 text-white text-xs rounded-xl px-3 py-2 border border-slate-700 outline-none focus:border-emerald-500"/>
        ...
        {prescOpen && userRole === 'doctor' && (<div className="absolute top-0 left-0 bottom-0 w-84 bg-slate-900/95 backdrop-blur-lg border-r border-slate-800 p-4 flex flex-col z-20 shadow-2xl">
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              <h4 className="font-bold text-xs text-white flex items-center gap-1.5">
            ...
            <form onSubmit={handleIssuePrescription} className="flex-1 flex flex-col justify-between py-3 space-y-4">
              <div className="space-y-3 text-xs text-slate-300">
                ...
                <div>
                  ...
                  <textarea rows={4} value={prescNotes} onChange={e => setPrescNotes(e.target.value)} placeholder="e.g. Tab Sorbilate 5mg 1-0-1 under tongue. Refer to IGMC Cath Lab for 2D Echo..." className="w-full bg-slate-800 text-white text-xs rounded-xl p-3 border border-slate-700 outline-none focus:border-emerald-500 resize-none"/>
                </div>
              </div>

              {prescSuccess ? (<div className="bg-emerald-950 text-emerald-300 p-3 rounded-xl border border-emerald-800 flex items-center gap-2 text-xs font-bold">
                  <CheckCircle className="h-5 w-5 text-emerald-400"/>
                  <span>E-Prescription signed & attached to ABHA thread!</span>
                </div>) : (<Button type="submit" variant="primary" className="w-full">
      ...
      <div className="h-20 bg-slate-900/90 backdrop-blur-md border-t border-slate-800 px-6 flex items-center justify-between z-20">
        ...
        <div className="flex items-center gap-3">
          {/* Mute Mic */}
          <button onClick={() => setMicActive(!micActive)} className={`p-3.5 rounded-2xl border transition-all ${micActive
            ? 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'
            : 'bg-rose-600 border-rose-500 text-white animate-pulse'}`} title={micActive ? "Mute Microphone" : "Unmute Microphone"}>
            {micActive ? <Mic className="h-5 w-5"/> : <MicOff className="h-5 w-5"/>}
          </button>

          {/* Toggle Camera */}
          <button onClick={() => setCameraActive(!cameraActive)} className={`p-3.5 rounded-2xl border transition-all ${cameraActive
            ? 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'
            : 'bg-rose-600 border-rose-500 text-white animate-pulse'}`} title={cameraActive ? "Turn Off Camera" : "Turn On Camera"}>
            {cameraActive ? <VideoIcon className="h-5 w-5"/> : <VideoOff className="h-5 w-5"/>}
          </button>

          {/* Screen Share */}
          <button onClick={() => setScreenSharing(!screenSharing)} className={`p-3.5 rounded-2xl border transition-all hidden sm:flex ${screenSharing
            ? 'bg-emerald-600 border-emerald-500 text-white'
            : 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'}`} title="Share Screen / Diagnostic ECG">
            <Monitor className="h-5 w-5"/>
          </button>

          {/* In-Call Chat */}
          <button onClick={() => setChatOpen(!chatOpen)} className={`p-3.5 rounded-2xl border transition-all relative ${chatOpen
            ? 'bg-emerald-600 border-emerald-500 text-white'
            : 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'}`} title="Open Live Chat">
          ...
          {userRole === 'doctor' && (<button onClick={() => setPrescOpen(!prescOpen)} className={`p-3.5 rounded-2xl border transition-all ${prescOpen
                ? 'bg-emerald-600 border-emerald-500 text-white'
                : 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700'}`} title="Issue E-Prescription">
              <FileText className="h-5 w-5"/>
            </button>)}

          {/* End Call Button */}
          <button onClick={onEndCall} className="p-3.5 px-6 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-bold flex items-center gap-2 shadow-lg transition-all" title="End Teleconsultation">
