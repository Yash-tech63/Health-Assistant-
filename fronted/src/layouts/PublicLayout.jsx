          ...
          <div className="bg-gradient-to-r from-rose-600 via-pink-600 to-rose-700 text-white py-2 px-4 text-xs">
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {rolesList.map(r => (
                  <button key={r.role} onClick={() => handleQuickSwitch(r.role)} className={`px-2 py-1 rounded bg-white/10 hover:bg-white/20 transition-all font-semibold flex items-center gap-1 ${user?.role === r.role ? 'ring-2 ring-white bg-white/20' : ''}`}>
                    <span>{r.icon}</span>
                    <span>{r.label}</span>
                  </button>
                ))}
            ...
            <div className="flex justify-between h-16 items-center">
              ...
              <nav className="hidden md:flex space-x-8">
                {navigation.map((item) => {
                  ...
                  return (
                    <Link key={item.name} to={item.href} className={`text-sm font-medium transition-colors ${isActive
          ...
          {mobileMenuOpen && (
            <div className="md:hidden border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-3 space-y-3 shadow-lg transition-all">
              <nav className="flex flex-col space-y-2">
                {navigation.map((item) => (
                  <Link key={item.name} to={item.href} onClick={() => setMobileMenuOpen(false)} className="px-3 py-2 rounded-lg text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900 hover:text-medical-600 dark:hover:text-medical-400">
                    {item.name}
                  </Link>
                ))}
              </nav>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
                ...
                <div className="grid grid-cols-2 gap-2 px-2">
                  {rolesList.map(r => (
                    <button key={r.role} onClick={() => handleQuickSwitch(r.role)} className="text-xs py-2 px-2.5 rounded-lg border border-slate-200 dark:border-slate-800 text-left bg-slate-50 dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-medium flex items-center gap-1.5 hover:bg-medical-50 dark:hover:bg-medical-950/20">
                      <span>{r.icon}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  <li><Link to="/assistant" className="hover:text-white transition-colors">Personal Health Assistant</Link></li>
