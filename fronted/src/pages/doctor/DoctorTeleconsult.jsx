...
export const DoctorTeleconsult = () => {
    ...
    if (inCall && activePatient) {
        return (<VideoConsultationRoom doctorName={user?.name || 'Dr. Arvind Sharma'} doctorSpecialty={user?.specialty || 'General Medicine & Cardiology'} doctorFacility={user?.facilityName || 'Shimla District General Hospital'} patientName={activePatient.name} abhaId={activePatient.abhaId} userRole="doctor" onEndCall={() => setInCall(false)}/>);
    }
      ...
      <div className="space-y-4">
        ...
        <Card>
          <CardBody className="p-0">
            ...
            return (<tr key={pat.id} className={`hover:bg-slate-50/50 dark:hover:bg-slate-900/10 cursor-pointer ${isSelected ? 'bg-emerald-50/30 dark:bg-emerald-950/20' : ''}`} onClick={() => setSelectedPatient(pat)}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            {pat.avatar ? (<img src={pat.avatar} alt={pat.name} className="h-10 w-10 rounded-full object-cover border border-slate-200 shrink-0"/>) : (<span className="text-2xl">👨‍🌾</span>)}
                            <div>
                        ...
                        <td className="px-6 py-4">
                          {idx === 0 ? (<Badge color="success" className="animate-pulse">
                              🟢 Next In Line
