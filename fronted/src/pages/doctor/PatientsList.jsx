...
import { Search, FileText, Plus, ClipboardCheck, Heart } from 'lucide-react';
...
export const PatientsList = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="max-w-md bg-white dark:bg-slate-800 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700/50 shadow-xs">
        <Input id="pat-list-search" placeholder="Search by patient name or ABHA number..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} leftIcon={<Search className="h-4 w-4"/>}/>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                {filteredPatients.map(p => (<tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2.5">
                        {p.avatar ? (<img src={p.avatar} alt={p.name} onError={(e) => {
                ...
                }} className="h-10 w-10 rounded-full object-cover border border-slate-200 dark:border-slate-800 shadow-xs shrink-0"/>) : (<span className="text-2xl">👨‍🌾</span>)}
                        <div>
      ...
      {activePatient && (<Modal isOpen={!!activePatient} onClose={() => setActivePatient(null)} title={`E-Health Profile: ${activePatient.name}`} size="lg">
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="border border-slate-100 dark:border-slate-800 rounded-xl divide-y divide-slate-100 dark:divide-slate-800">
                {diagnostics.filter(d => d.patientId === activePatient.id).map(diag => (<div key={diag.id} className="p-3 text-xs flex justify-between items-center bg-white dark:bg-slate-800/30">
                    <div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                {prescriptions.filter(p => p.patientId === activePatient.id).map(presc => (<div key={presc.id} className="border border-slate-150 dark:border-slate-800 rounded-xl p-3.5 text-xs bg-slate-50/50 dark:bg-slate-900/10">
                    ...
                    <ul className="list-disc list-inside space-y-1.5 text-slate-650 dark:text-slate-350">
                      {presc.medicines.map((med, id) => (<li key={id}>
      ...
      {prescModalOpen && activePatient && (<Modal isOpen={prescModalOpen} onClose={() => setPrescModalOpen(false)} title={`Write E-Prescription: ${activePatient.name}`} size="lg">
          <form onSubmit={handlePrescribeSubmit} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {newMedicines.map((med, idx) => (<div key={idx} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-end border border-slate-100 dark:border-slate-850 p-3 rounded-lg bg-slate-50/30 dark:bg-slate-900/10">
                  <div className="sm:col-span-1.5">
                    <label className="block text-[9px] text-slate-500 mb-0.5">Medicine Name</label>
                    <input type="text" placeholder="e.g. Metformin 500mg" value={med.name} onChange={e => handleMedChange(idx, 'name', e.target.value)} className="w-full text-xs rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2 outline-none" required/>
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-500 mb-0.5">Dosage Pattern</label>
                    <input type="text" placeholder="e.g. 1-0-1" value={med.dosage} onChange={e => handleMedChange(idx, 'dosage', e.target.value)} className="w-full text-xs rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2 outline-none" required/>
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-500 mb-0.5">Duration</label>
                    <input type="text" placeholder="e.g. 10 days" value={med.duration} onChange={e => handleMedChange(idx, 'duration', e.target.value)} className="w-full text-xs rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2 outline-none" required/>
                  </div>
                  <div>
                    <label className="block text-[9px] text-slate-500 mb-0.5">Instructions</label>
                    <input type="text" placeholder="e.g. Post meals" value={med.instructions} onChange={e => handleMedChange(idx, 'instructions', e.target.value)} className="w-full text-xs rounded border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-2 outline-none"/>
                  </div>
                </div>))}
            ...
            <div>
              ...
              <textarea rows={3} value={prescNotes} onChange={e => setPrescNotes(e.target.value)} placeholder="Diagnostic summary, blood pressure check findings, follow-up advice..." className="w-full text-xs rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 p-3 outline-none focus:ring-2 focus:ring-medical-500"/>
            </div>
