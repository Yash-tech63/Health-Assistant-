...
export const Chat = () => {
    ...
    const chatMessages = messages.filter(m => (m.senderId === patientId && m.receiverId === activeDoctorId) ||
        (m.senderId === activeDoctorId && m.receiverId === patientId));
    const handleSend = (e) => {
        e.preventDefault();
        if (!inputText.trim() || !activeDoctor)
            return;
        ...
        setTimeout(() => {
            sendMessage(activeDoctorId, activeDoctor.name, patientId, "Dhanyawaad for your message. I have logged this check in your E-Record. We will review this during our follow-up consultation.");
        }, 1500);
    };
      ...
      <Card className="h-[550px] flex flex-col overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-4 h-full divide-x divide-slate-100 dark:divide-slate-850">
          ...
          <div className="p-4 space-y-3 bg-slate-50/50 dark:bg-slate-950/20 col-span-1">
            ...
            {doctors.map(doc => (<button key={doc.id} onClick={() => setActiveDoctorId(doc.id)} className={`w-full text-left p-3 rounded-xl border text-xs transition-all flex items-center gap-2 ${activeDoctorId === doc.id
          ...
          <div className="flex flex-col col-span-3 h-full">
            {activeDoctor ? (<>
                ...
                return (<div key={msg.id} className={`flex ${isSelf ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-xs sm:max-w-md rounded-2xl px-4 py-2.5 text-xs shadow-xs border ${isSelf
                  ...
                  {chatMessages.length === 0 && (<div className="text-center py-20 text-slate-400 text-xs">
                ...
                <form onSubmit={handleSend} className="p-4 border-t border-slate-150 dark:border-slate-850 bg-slate-50/20 dark:bg-slate-900/10 flex gap-2">
                  <Input id="chat-text" value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Type message here..." className="flex-1" autoComplete="off" required/>
