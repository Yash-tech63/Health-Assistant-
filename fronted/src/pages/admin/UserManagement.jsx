...
export const UserManagement = () => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="max-w-md bg-white dark:bg-slate-800 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700/50 shadow-xs">
        <Input id="user-search" placeholder="Filter doctors by name or specialty..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} leftIcon={<Search className="h-4 w-4"/>}/>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            ...
            return (<tr key={doc.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
                        ...
                        if (parent) {
                    ...
                    }} className="h-9 w-9 rounded-full object-cover border border-slate-200 dark:border-slate-800 shadow-xs shrink-0"/>) : (<span className="text-2xl">👨‍⚕️</span>)}
                              <div>
                          ...
                          <td className="px-6 py-4 text-right">
                            <Button variant={isVerified ? 'success' : 'danger'} size="sm" onClick={() => handleToggleVerify(doc.id)} leftIcon={isVerified ? <CheckCircle className="h-3.5 w-3.5"/> : <ShieldAlert className="h-3.5 w-3.5"/>}>
          ...
          <Card>
            <CardBody className="p-0">
              <div className="divide-y divide-slate-100 dark:divide-slate-850">
                {facilities.map(fac => (<div key={fac.id} className="p-4 space-y-2 text-xs hover:bg-slate-50/50 dark:hover:bg-slate-900/10">
